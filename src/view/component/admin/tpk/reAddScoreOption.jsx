 ...
 * @Last Modified time: 2019-03-12 11:17:44
 */
import React, { Component } from 'react';
import _ from 'lodash';
import { Input, Form, InputNumber, Button, message } from 'antd';
import { SVG } from './../../../../base';
import './../../../../css/admin/mj_reAddScoreOption.css';
...
class ReAddScoreOption extends Component {
  ...
  remove = (k) => {
    ...
    if (keys.length === 1) {
      message.info('提示框', '请至少保留一个评分子项', 2000);
      return;
    }
    ...
    if (this.total > 100) {
      ...
      this.props.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (isRepeat) {
            message.info('提示框', '评价子项名重复', 2000);
          } else {
            var commit = true;
            if (values.evaluateModelName.trim() === '') {
              message.info('提示框', '请填写评价大项名');
              commit = false;
              return false;
            } else {
      ...
      return (
        <div className="cjy-raso-childOpt" key={k}>
          <div className="cjy-raso-titleLine">
             <SVG type='untitled' onClick={() => this.remove(k)} />
            &nbsp;
            <span>评分子项：</span>
            <FormItem>
