 ...
 * @Last Modified time: 2019-03-18 13:41:31
 * 异常报警
 */
...
import { ll_tabs, ll_Opear, ll_search, ll_uidList, ll_img } from '../../../../../redux/warn.reducer'
...
export default class LL_WarnTable extends Component {
    ...
    componentDidMount() {
        console.log('his.props.ll_warn.tabs.tab', this.props.ll_warn.tabs.tab)
        console.log('this.props.ll_warn.search', this.props.ll_warn.search)
        this.change()

    }

    change() {
        this.props.ll_search(this.props.ll_warn.search)
        let { data } = this.state
        if (this.props.ll_warn.tabs.tab === '4') {
            for (var i = 0; i < 22; i++) {
                data.push({
                    key: i,
                    time: '第一周/周一/第二节',
    ...
    render() {
        ...
        let ll_warn = this.props.ll_warn;
        let arr = [
            {
                'classIndex': '一',
                'Unapproved': 10,
        ...
        const columns = this.props.ll_warn.tabs.tab === '4' ?
            [{
                title: '时间',
                dataIndex: 'time',
                width: '174px',
        ...
        //批量选择
        const rowSelection = {
            selectedRowKeys: ll_warn.uidList.selectedRowKeys,
        ...
        return (
            < div className='ll-tableBody'>
                ...
                <div style={{ display: 'inline-block' }}>
                    时间筛选： <RangePicker onChange={this.SearchChange.bind(this, 'dateTime')} value={ll_warn.search.dateTime.length ? [moment(ll_warn.search.dateTime[0], dateFormat), moment(ll_warn.search.dateTime[0], dateFormat)] : null} format={dateFormat} />
                </div>
                <div id={ID} style={{ display: 'inline-block' }}>
                    <Select value={ll_warn.search.dropType} style={{ width: 100, marginLeft: '10px' }} className='ll-tableSelect' onChange={this.SearchChange.bind(this, 'dropType')} getPopupContainer={() => document.getElementById(ID)}>
                ...
                <Search
                    ...
                    style={{ width: 200 }}
                    defaultValue={`${ll_warn.search.inputTeacherName}`}
                />

                <div className='ll-wzyTop'>
                    <Row type="flex" justify="space-between">
                        <Col span={4} className={-1 === ll_warn.search.festivals ? 'll-changeColor' : ''}>
                            <div>
                                <span>未审定</span>/
                                <span>正常</span>/
                                <span>异常</span>
                        ...
                        </Col>
                        {
                            arr.map(function (data, key) {
                                return (
                                    <Col span={2} key={key} className={key === ll_warn.search.festivals ? 'll-changeColor' : ''}>
                ...
                <div className='ll-buttom'>
                    ...
                    <Pagination
                        current={ll_warn.search.pageNowe}
                        // defaultCurrent={1}
