 ...
 * @Last Modified time: 2019-03-15 15:11:23
 * 异常报警
 */
import React, { Component } from 'react';
import { Tabs, Button, Select, Input, Table, Pagination, Row, Col, Modal, InputNumber } from 'antd';
...
@connect(state => state, { ll_tabs, ll_Opear, ll_ExaminationPass, ll_Initialization, ll_ExaminationError, ll_uidList, ll_search,ll_img })
export default class LL_Warining extends Component {
  ...
  componentDidMount() {
    ...
    // this.props.ll_warnTea.tabMessage.page=1;
    // this.props.ll_warnTea.tabs.tab='1';
    ...
    this.setState({
      arr,
      search: this.props.ll_warnTea.search
    })
  }
  ...
  PageBack = () => {
    ...
    console.log(this.props.ll_warnTea.search)
    console.log(this.props.ll_warnTea.uidList)
  ...
  jumpPage(pageIndex) {
    ...
    this.props.ll_img({
      uid: this.props.ll_warnTea.img.uid,
      pageNow: pageIndex,
      pageSize: "20"
    })
  ...
  CheckExamination(type) {
    switch (type) {
      case 'ExaminationPass':
        let examinationPass = {
          "type": this.props.ll_warnTea.tabs.tab, //[1:调课换课;2教师迟到;3学生出勤]
          "uidList": this.props.ll_warnTea.uidList //[List类型]操作对象uid(课程id)"
        }
        ModalTip({
          tit: '系统提示',
          ctn: '确定数据通过？',
          oT: '确定',
          okFun: () => {
            this.props.ll_ExaminationPass(examinationPass)
            this.props.ll_uidList({
              checkedUidList: [],//[List类型]操作对象uid(课程id)"
              selectedRowKeys: []
            })//清空uidlist
            let search = this.props.ll_warnTea.search;
            search.pageNowe = 1;
      ...
      case 'ExaminationError':
        ...
        ModalTip({
          ...
          oT: '确定',
          okFun: () => {
            this.props.ll_ExaminationError(examinationError)
            this.props.ll_uidList({
              checkedUidList: [],//[List类型]操作对象uid(课程id)"
              selectedRowKeys: []
            })
  ...
  render() {
    let tabMessage = this.props.ll_warnTea.tabMessage;
    let tabs = this.props.ll_warnTea.tabs;
    let search = this.props.ll_warnTea.search;
    let uidList = this.props.ll_warnTea.uidList;
    let img = this.props.ll_warnTea.img
