 ...
 * @Last Modified time: 2018-11-05 14:20:50
 */
import {
    message
} from 'antd';
...
export function showAddModal1(flag) {
    return dispatch => {
        if (!flag) {
            ...
            dispatch({
                type: ZN_ADD_START,
                data: new Date().setHours(0, 0, 0, 0)
            });
            // 置空结束时间
            dispatch({
                type: ZN_ADD_END,
                data: new Date().setHours(23, 59, 59, 59)
            });
            // 置空报告名称
            dispatch({
...
//多选的checkbox
export function znCheck_report(v) {
    return dispatch => {
        dispatch({
            type: ZN_OPTIONS,
            data: v
 ...
 */
export function addReportMsg(name, startDate, endDate, json, send) {
    return dispatch => {
        console.log(name, startDate, endDate, json)
        if (send === '1') {
            if ((name && startDate && endDate && json === 'light') && name !== '') {
                dispatch({
                    type: ZN_ADD_STATUS,
                    payload: true,
                })
                return;
            } else if ((name && startDate && endDate && json === 'noLight') || name === '') {
                dispatch({
                    type: ZN_ADD_STATUS,
                    payload: false,
                })
...
// 新增报告请求
export function zn_add_ajax(params) {
    return dispatch => {
        console.log(this);
