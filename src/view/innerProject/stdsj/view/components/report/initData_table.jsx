 ...
 * @Last Modified time: 2019-03-25 15:51:33
 */
...
class InitDataTable extends React.Component {
      ...
      this.setState({
        ...
        if (this.props.comp === "课堂秩序原始数据") {
          ...
          this.initDataTable({
            ...
            "collegeId": this.state.isChecked1 ? "" : (this.state.isChecked2 ? this.state.collegeXYId : ""), //学院ID//
        ...
        this.setState({
          ...
          this.setState({
            ...
            this.zlInitDataTable({
              "startTime": new Date(this.state.selDate3).setHours(0, 0, 0, 0), //开始时间 
              "endTime": new Date(this.state.selDate4).setHours(23, 59, 59, 59), //结束时间
   ...
   */
  xyPeople = (params) => {
    _x.util.request.request(this.props.comp === "课堂秩序原始数据" ? 'api/web/common/get_all_patroler' : 'api/web/common/get_all_commander', params, (res) => {
      if (res.result && res.data) {
        // console.log(res);
        this.setState({
          xyPeople: res.data,
   ...
   */
  xxPeople = () => {
    _x.util.request.request(this.props.comp === "课堂秩序原始数据" ? 'api/web/common/get_all_patroler' : 'api/web/common/get_all_commander', {}, (res) => {
      if (res.result && res.data) {
        // console.log(res);
        this.setState({
          xxPeople: res.data,
      ...
      if (this.state.college[i].trgId === this.state.collegeVal) {
        this.setState({
          ...
          let params = {
            ...
            "recorder":
              // G.isVer === "0" ? 
              // this.state.xyPeopleVal :
              (this.state.isChecked1 ? this.state.xxPeopleVal : this.state.xyPeopleVal),//记录人标识 如果查询所有 传" "//
            "pageSize": this.state.pageSize,//每页显示条数//
            "pageIndex": this.state.page//当前页//
          }
          console.log(this.state.collegeXYName);
          console.log(params.collegeId)
      ...
      if (this.state.college[i].trgId === this.state.collegeVal) {
        this.setState({
          ...
          let params = {
            ...
            "collegeId": this.state.isChecked1 ? "" : (this.state.isChecked2 ? this.state.collegeXYId : ""), //学院ID//
            "scoreMan":
              // G.isVer === "0" ? this.state.xyPeopleVal : 
              (this.state.isChecked1 ? this.state.xxPeopleVal : this.state.xyPeopleVal),//记录人标识 如果查询所有 传" "//
            "pageSize": this.state.pageSize1,//每页显示条数//
            "pageIndex": this.state.page1//当前页//
          }
      ...
      if (res.result && res.data) {
        ...
        this.setState({
          ...
          if (this.props.comp === "课堂秩序原始数据") {
            ...
            console.log('课程22222222222222', this.state.course);
            if (this.props.comp !== "课堂秩序原始数据") {
              this.setState({
                course: res.data,
              }, () => {
        ...
        for (let i = 0; i < this.state.collegeXY.length; i++) {
          if (this.state.collegeXY[i].trgId === this.state.collegeXYVal) {
            this.setState({
              ...
              collegeXYId: this.state.collegeXY[i].trgId
            }, () => {
              console.log(this.state.collegeXYName);
              this.xyPeople({ "trgId": this.state.collegeXYId });
            })
          }
        };
      } else {
  ...
  collegePeo = (e) => {
    this.setState({
      ...
      if (this.props.comp === "课堂秩序原始数据") {
        ...
        console.log(this.state.xyPeopleVal);
        this.ReqTableAll1();
      }
    })
  }
  ...
  render() {
    ...
    const columns =
      ...
      //   render: (text) => { return text === 1 ? "学生" : (text === 2 ? "教师" : "教师/学生") },
      //   // text === 1 ? "学生" : "老师",
      //   width: 140,
      //   align: 'left',
      // }, {
      ...
      [{
        ...
        dataIndex: 'type',
        width: 140,
        render: (text) => text === 1 ? "公共基础" : (text === 2 ? "公共选修" : (text === 3 ? "专业基础" : "专业选修")),
        align: 'left',
      }, {
        title: '违纪对象',
        dataIndex: 'roleEventType',
            ...
            this.props.comp === "课堂秩序原始数据" ?
              ...
              <div className="kyl-crc-HContent">
                ...
                <div className="kyl-crc-Hpadding">
                  {
                    ...
                    //   ?
                    //   <div style={{ display: 'inline-block' }}>
                    //     <span style={{ marginLeft: "-14px" }}>巡课员&nbsp;:&nbsp;</span>
                    //     {/* 记录人 ---学院 */}
                    //     <Select
                    //       filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                ...
                <div className="kyl-crc-Hpadding">
                  ...
                  <Button style={{ marginLeft: '30px' }}>查询</Button>
                </div>
              </div>
              :
              // 课堂质量
                ...
                <div className="kyl-crc-Hpadding">
                  ...
                  {/* 开课学院 */}
                  {
                    ...
                    <Select
                      ...
                      {
                        this.state.kkxyjg.map((item, index) => (
                          <Option title={item.orgName} key={index} value={item.orgId}>{item.orgName}</Option>
                        ))
                      }
                    </Select>
                  }
                  ...
                  }
                  {
                    ...
                    <div style={{ display: "inline-block" }}>
                      ...
                      <Select
                        ...
                        showSearch={true}
                        style={{
                          width: 150, marginLeft:
                            //  G.isVer === "0" ? 0 : 
                            "15px",
                          display: this.state.isChecked1 && this.state.isChecked2 || this.state.isChecked1 ? "inline-block" : "none"
                        }}
                        getPopupContainer={() => document.getElementsByClassName('kyl-crc-HContent')[0]}
                        onChange={this.schoolPeo}>
                        {
