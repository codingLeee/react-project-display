 ...
 * @Last Modified time: 2019-03-20 15:54:03
 * 学生出勤报表-原始数据组件
 */
...
class StuRepData extends Component {
    ...
    componentDidMount() {
        ...
        colList.length && this.getClassroomData(colList[0].trgName, colList[0].trgId);
        colList.length && this.getTeacherData(openTrgList[0].orgId, typeList[0].id);
     ...
     * 获取学院对应的班级数据
     */
    getClassroomData(trgName, trgId) {
        let param = {
            trgName: trgName,
            trgId: trgId
        }
    ...
    onChangeCol = (ind) => {
        ...
        this.props.updateDataParams(params);
        // 获取学院对应的班级数据
        this.getClassroomData(colList[ind].trgName, colList[ind].trgId);
    }
    ...
    downloadFile = () => {
        Request('api/web/stu_report/export_original_report', this.props.studentReducer.dataParam, (res) => {
            if (res.result && res.data) {
                let downUrl = G.serverUrl + res.data;
