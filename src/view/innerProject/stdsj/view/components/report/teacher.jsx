 ...
 * @Last Modified time: 2019-03-12 10:37:49
 * 教师考勤-教师报表组件
 */
...
export default class Teacher extends React.Component {
  ...
  componentDidMount() {
    ...
    // 获取老师列表
    this.getTeacherList(colList.length ? colList[0].trgId : 'all')
    // // 更新入参
  ...
  getTeacherList(trgId) {
    ...
    Request('api/web/common/college_tea_dropdown', param, (res) => {
      let data = res.data;
      if (res.result && data) {
        this.setState({
          teaList: data,
  ...
  onChangeCol = (ind) => {
    ...
    this.updateDataParams(params);
    // 获取老师数据
    this.getTeacherList(colList[ind].trgId);
  }
   ...
   */
  onChangeCol1 = (ind) => {
    let teaList = this.state.teaList;
    this.setState({
      teaIndex: ind
    });
    let params = this.state.params;
    params.teacherId = teaList[ind].teacherId;
    params.pageIndex = 1;
    this.setState({
      courPage: 1,
    })
      ...
      <div className="zn-bg" ref={(ref) => this.node = ref}>
        ...
        <div className="lxx-hd-g-rg">
          <Table
            ...
            <Column title="考勤异常次数" dataIndex="unusualNumber" key="unusualNumber" sorter={true} width={140}></Column>
          </Table>
          {
            !state.total
              ? ''
