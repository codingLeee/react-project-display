 ...
 * @Last Modified time: 2019-03-27 16:16:45
 * 可视化中心-学生出勤
 */
...
class ClassroomStu extends React.Component {
  ...
  req = () => {
    // 获取本周、本月、本学期概览
    this.requiretimedata(1);
    this.requiretimedata(2);
    this.requiretimedata(3);
  ...
  requiretimedata=(typeId)=>{
    //概览数据
    Request('api/web/visual_student/get_attendence_by_type',{type:typeId},res=>{
      ...
      if(typeId===1){
        ...
        for (let i = 0; i < week.length; i++) {
                    weekData.push(week[i].yData);
                    weekAxis.push('周' +week[i].xData);
                    weekNumber.push(week[i].totalNum);
                  }
      ...
      if(typeId===2){
        ...
        for (let j = 0; j < month.length; j++) {
              monthData.push(month[j].yData)
              monthAxis.push('第'+month[j].xData+'周')
              monthNumber.push(month[j].totalNum);
                  }
      ...
      if(typeId===3){
        ...
        for (let k = 0; k < total.length; k++) {
                     termData.push(total[k].yData);
                     termAxis.push(total[k].xData + '月');
                    termNumber.push(total[k].totalNum);
                   }
    ...
    // Request(
    //   'api/web/visual_student/get_attendence_by_type',
    //   {type:1}
    //   , (res) => {
    //     this.setState({ LineChartLoading: false })
    //     if (res.result) {
  ...
  require = () => {
    //今日总览数据请求
    this.setState({ todayAllLoading: true, LineChartLoading: true })
    _x.util.request.request(
      'api/web/visual_student/get_overview',
      {}
      , (res) => {
  ...
  //今日违纪数据请求
  tableData = () => {
    this.setState({ totayDisLoading: true })
    // _x.util.animation.remove(loopkey);
    clearInterval(loopkey);
    _x.util.request.request(
      'api/web/visual_student/get_stu_unnormal',
      {},
      (res) => {
        if (res.result) {
  ...
  attData = () => {
    ...
    //出勤排行数据
    _x.util.request.request(
      'api/web/visual_student/get_attendence_order',
      {
        ...
        if (res.result) {
          if (res.data) {
            ...
            for (let i = 0; i < res.data.length; i++) {
              attData.push(res.data[i].yData);
              attAxis.push(res.data[i].xData);
              attNumber.push(res.data[i].totalNum);
            }
  ...
  teaData = () => {
    ...
    _x.util.request.request(
      'api/web/visual_student/get_teacher_order',
      {
        type: this.state.checked,
  ...
  courData = () => {
    ...
    _x.util.request.request(
      'api/web/visual_student/get_course_order',
      {
        type: this.state.checked,
      ...
      , (res) => {
        ...
        if (res.result) {
          if (res.data) {
            ...
            for (let i = 0; i < res.data.length; i++) {
              courData.push(res.data[i].yData);
              courAxis.push(res.data[i].xData || '-');
              courNumber.push(res.data[i].totalNum);
            }
    ...
    const title = [
      {
        ...
        name: '出勤率',
        score: this.state.averageAttendence || this.state.averageAttendence === 0 ? <div>{this.state.averageAttendence}<span style={{ fontSize: '16px' }}>%</span></div> : '-',
        background: '#783ec8'
      },
    ];
                          ...
                          <tbody ref={re => this.teaTableChild = re}>
                            {
                              this.state.data.map((item, i) => {
                                return <tr key={i}>
                                  ...
                                  <td><span>{item.attendanceRate}%</span></td>
                                </tr>
                              })
                            }
                          </tbody>
