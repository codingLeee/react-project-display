 ...
 * @Last Modified time: 2019-03-22 10:39:16
 * 可视化中心-教师考勤
 */
...
class ClassroomTea extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      order:0,                //考勤异常类型次数排行;1表示升序，-1表示降序
  ...
  requiretimedata=(typeId)=>{
    //概览数据
    _x.util.request.request('api/web/visual_student/get_attendence_by_type',{type:typeId},res=>{
  ...
  require = () => {
    ...
    _x.util.request.request(
      'api/web/visual_teacher/get_overview',
      {}
      , (res) => {
    ...
    //考勤异常次数数据
    _x.util.request.request(
      'api/web/visual_teacher/get_attendence_by_type',
  ...
  typeData = () => {
    ...
    _x.util.request.request(
      'api/web/visual_teacher/get_type_unnormal',
      {
        type: this.state.checked,
  ...
  teaData = () => {
    ...
    _x.util.request.request(
      'api/web/visual_teacher/get_tea_unnormal',
      {
        type: this.state.checked,
      ...
      , (res) => {
        ...
        if (res.result) {
          if (res.data) {
            ...
            for (let i = 0; i < res.data.length; i++) {
              teaData.push(Math.floor(res.data[i].yData));
  ...
  order = () => {
    if (this.state.order === 1) {
      this.setState({ order: 0 }, () => this.typeData())
    } else {
  ...
  aveOrder = () => {
    if (this.state.aveOrder === 1) {
      this.setState({ aveOrder: 0 }, () => this.teaData())
    } else {
  ...
  handleStart = (type) => {
    ...
    } else if (type === 0 && this.state.changeData.length > 4) {
      this.handleScroll(type);
    }
  }
                    ...
                    this.state.changeData.length ?
                      <div className='zq-comn-table xt-tea-table'>
                        <table ref={re => this.teaTable2 = re} onMouseEnter={() => this.handleStop(loopkey2)}
                          onMouseLeave={() => this.handleStart(0)} className='zq-stabl-table' style={{ height: this.state.height * 0.25 + 'px' }}>
