 ...
 * @Last Modified time: 2019-03-26 13:40:00
 * 公共组件-笔记本
 */
import React, { Component } from 'react';
import {Button,Input,Select} from 'antd';
import { confirm,success,error,warning,info} from '../base/modal.jsx';
import DiaryNote from '../base/diaryNote.jsx';
...
class Diary extends Component {
  ...
  disabledNextPre(){
   ...
   getWeekAndClassDataLis() {
    ...
    _x.util.request('/teacherJob/selectMyListenClass',req,function(ret){
      if(ret.result){
        ...
        if(resData.listenClass.length==0){
          ...
          classID=resData.listenClass[0].classID||'';   
        }   
         let haveWeek=[];
         let cIndex=1;
  ...
  getLisNoteData(req){ 
    ...
    //   classID:classid,
    // };
    _x.util.request('/teacherJob/selectMyAllListenNote',req,function(ret){
      if(ret.result){
        let resData = ret.data;
        let resultdata=[];
        if(JSON.stringify(resData) !== "{}"){
  ...
  getCurriNote(){
    ...
    _x.util.request('/teacherJob/selectMyCurriNote',req2,function(ret){
      if(ret.result){
        let resData = ret.data;
        let resultdata=[];
        let classes=[];
  ...
  getCurriListenNote(){ 
    ...
    _x.util.request('/teacherJob/selectMyListenClass',req,function(ret){
      if(ret.result){
        ...
        localStorage.setItem('weeks',JSON.stringify(haveWeek));     
        localStorage.setItem('classID',JSON.stringify(classID));    
        var index = _.findIndex(haveWeek, {key: Global.currentWeek});
        this.setState({
          weeksData:resData.listenWeeks,
          weeksList:haveWeek,
          techClass:allClass||[],
    ...
    _x.util.request('/teacherJob/selectMyCurriListenNote',req2,function(ret){
      if(ret.result){
        let resData = ret.data;
        let resultdata=[];
        let classes=[];
  ...
  //教研本-搜索功能请求数据
  getSearchMyNote(req){
    _x.util.request('/teacherJob/selectSearchMyNote',req,function(ret){
      if(ret.result){
        let resData = ret.data;
        if(!resData.curriculumall.length==0){
          let weekdata =JSON.parse(localStorage.getItem('weeks'));
  ...
  getLisSearchMyNote(req){
    _x.util.request('/teacherJob/selectSearchMyListenNote',req,function(ret){
      if(ret.result){
        let resData = ret.data;
        if(!resData.curriculumall.length==0){
          let weekdata =JSON.parse(localStorage.getItem('weeks'));
    ...
    _x.util.request('/teacherJob/selectSearchMyNote',req,function(ret){
      if(ret.result){
        ...
        if(!resData.curriculumall.length==0){
          ...
          info('提示','请输入存在于教研计划中的授课者姓名！',3000);
          this.setState({
            disabledNext:true,//翻页禁用
            disabledPre:true,//翻页禁用
            curriculumall:[],
    ...
    _x.util.request('/teacherJob/selectSearchMyListenNote',req,function(ret){
      if(ret.result){
        ...
        if(!resData.curriculumall.length==0){
          ...
          info('提示','请输入存在于听课指标任务计划的授课者姓名！',3000); 
          this.setState({
            disabledNext:true,//翻页禁用
            disabledPre:true,//翻页禁用
            curriculumall:[],
      ...
      <div className="pf-r-curricululinfo">
        <div className="pf-dia-curriculultitle">
          <div className='pf-dia-classselect'>
            ...
            <Select showSearch value={devalue} style={{width:'160px'}} optionFilterProp="children" onChange={this.props.type=="research"?this.handleSelectSystem.bind(this,classes):this.handleSelectSystemLis.bind(this,classes)} 
          ...
          <Button size='large' className="pf-dia-pagepre" onClick={this.props.type=="research"?this.handleChangePre:this.handleChangePreLis} disabled={this.state.disabledPre}>上一页</Button>
          <Button size='large' className="pf-dia-pagenext" onClick={this.props.type=="research"?this.handleChangeNext:this.handleChangeNextLis} disabled={this.state.disabledNext}>下一页</Button>
          <Button size="large" className="pf-dia-bigbutton" onClick={this.props.type=="research"?this.handleSearch:this.handleSearchLis}><span>查 询</span></Button>
        ...
        <div className="pf-dia-curriculul">
          ...
          <div className="pf-dia-currweek">
            <ul>
              {
                this.state.weeksList.map((week,i)=><li key={i}><Button size='large'><a onClick={this.props.type=="research"?this.handleChangeWeek.bind(this,week,i):this.handleChangeWeekLis.bind(this,week,i)} className={this.state.clickIndex == i? 'clicked' : ''}>第{Number(week).toChinese()}周</a></Button></li> )
              }             
            </ul>
          </div>
        </div>
