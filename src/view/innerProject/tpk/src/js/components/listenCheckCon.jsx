 ...
 * @Last Modified time: 2017-10-09 16:35:01
 * 听评课-管理员部分-随堂设置-选择成员
 */
...
import ListenGroupName from '../components/base/listenGroupName';
...
import '../../css/components/listenCheckCon.css';
import '../../css/components/listenCheckedName.css';
...
class ListenCheckCon extends Component {
  ...
  //获取所有教研组
  requestGroups() {
    _x.util.request('/research_lesson/setting/get_all_research_groups', {}, function (ret) {
      if (ret.result) {
        const data = ret.data;
        data.map((item) => {
  ...
  handleOnDel(e) {
    ...
    const teaId = e.target.getAttribute('data-warning');
