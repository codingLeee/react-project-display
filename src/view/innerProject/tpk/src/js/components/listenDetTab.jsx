 ...
 * @Last Modified time: 2018-09-27 10:34:03
...
import { warning, info } from '../components/base/modal';
...
class ListenDetTab extends Component {
  ...
  componentDidMount() {
    ...
    if (!Global.loaded) {
      ...
      this.setState({
        weeks: Global.semester.weeks,
        seleSecValue: String(Global.currentWeek)
      })

      this.reuestTab(this.props.teaId, this.state.seleFirValue, String(Global.currentWeek), this.state.lisState, 1);
    }
  };
  ...
  reuestTab(teaName, startWeek, endWeek, lisStatus, pages) {
    ...
    _x.util.request('/listenJob/listenTeacherDetail', {
      'teacherId': teaName,
      ...
      if (ret.result) {
        ...
        data.map((item, index) => {
          ...
          var lisInfo = `${item.classroomName ? item.classroomName : '--'}/${item.className ? item.className : '--'}/${item.teachTeacherName ? item.teachTeacherName : '--'}/${item.courseName ? item.courseName : '--'}`;
          // var lisTime = `第${Number(item.weeks).toChinese()}周/星期${item.weekday === 7 ? '日' : Number(item.weekday).toChinese()}/第${Number(item.lessonOrder).toChinese()}节`;
  ...
  seleFirChange(value) {
    ...
    if (parseInt(value) <= parseInt(this.state.seleSecValue)) {
      ...
      info('操作提示', '开始周需小于等于结束周', function (resolve) {
        resolve();
      });
      this.setState({
        // seleFirValue: value,
  ...
  toPlay(record) {
    ...
    if (record.startTime > nowTime) {
      // console.log('未开始');
      info('提示框', '课程暂未开始,请在课程开始后观看', function (resolve) {
        resolve();
      });
    } else {
      const id = record.curriculumallId;
      // window.location.href = `${Global.rootpath}/resr1/${id}/2`;
      // window.open(`http://192.168.20.67:8080/cloud/home#resr2/${id}/2`);

      window.open(`${Global.rootpath}/home#resr2/${id}/2`);
    }
  }
        ...
        render: (text, record) => (
          <span>
            <a href="javascript:;" className="cjy-rld-aBox" onClick={() => this.toPlay(record)}>
              <i className="iconfont" title='播放'>&#xe609;</i>
            </a>
          </span>
        ),
      }
    ...
    return (
      <div className='mj-lot-tabCon'>
        ...
        <div className='mj-lot-operate'>
          ...
          <div className='mj-lft-seleCon'>
            ...
            <InputGroup compact className='mj-lft-sele mj-ldt-sele1'>
              <Select value={this.state.seleFirValue} onChange={this.seleFirChange}>
                {
            ...
            <InputGroup compact className='mj-lft-sele mj-ldt-sele2'>
              <Select value={this.state.seleSecValue} onChange={this.seleSecChange}>
                {
                  this.state.weeks.map((item, index) => (
          ...
          <div className='mj-lft-seleCon mj-ldt-seleCon'>
            ...
            <InputGroup compact className='mj-lft-sele'>
              <Select defaultValue={this.state.lisState} onChange={this.stateChange}>
        ...
        <div className='mj-lot-tab'>
          <Table columns={columns} loading={this.state.loading} dataSource={this.state.tabData} pagination={{ pageSize: 20, itemRender: itemRender, total: this.state.total, onChange: this.pageChange, current: this.state.curPage }} />
        </div>
