 ...
 * @Last Modified time: 2019-03-26 14:15:28
 * 已选教研课随堂听申请列表
 */
import React, { Component } from 'react';
import {Select,Button} from 'antd';
import { Link } from 'react-router-dom';
import { confirm,success,error,warning,info} from '../components/base/modal.jsx';
import { Global } from '../../js/base/g';
import _x from '../base/_x/api/api';
import _ from 'lodash';
import ResearchTree from './researchTree';
import ResearchSelect from './base/researchSelect';
import ResearchClassTable from './researchClassTable';
import ClassHaveChosed from './classHaveChosed';
import '../../css/components/reChoseLesson.css';
import '../../css/components/tasksApplyListenTable.css';
...
class ReChoseLesson extends Component{
  ...
  getLessonTable = (classId,curWeek,CurSstm) => {
    ...
    _x.util.request('/teacherJob/myChooseListen',req,function(ret){
      if(ret.message==="参数错误"){
        info('提示框','请选择要查看的班级',3000);
      }else{
        if(ret.result){
    ...
    _x.util.request('/research_lesson/setting/semester', {}, function (ret) {
      if (ret.result) {
        ...
        this.setState({
          ...
          week: newWeekData.length ? newWeekData[Global.currentWeek-1].value : ''       //周次下拉框设置当前默认选中第一条
        });
      }
    }.bind(this));
  };
  ...
  handleSubmit(){
    ...
    _x.util.request('/teacherJob/myApply',req,function(ret){
      if(ret.result){
        if(ret.data.flag){
          info('提示框','申请听课成功！',function(resolve){
            window.location.reload();
            resolve(); //执行此方法弹出框才会关闭，用于需要等待的异步操作
            });
        }else{
          info('提示框','申请听课失败，请重试！', 3000); 
        }
      }else{
        error('操作失败', 3000); 
      }
    ...
    info('提示框','您尚未选择随堂听课，请选择随堂听课后进行确认申请操作！',3000);
  }
  }
  ...
  handleSelectSystem(value){
    ...
    if(newWeekData.length){
      this.getLessonTable(this.state.classId,newWeekData[Global.currentWeek-1].value,value);
    }else{
      info('提示框','本学期暂无周次信息，请重新选择学期！',3000);
      this.setState({
        classInfo:[],
      });
    }
