 ...
 * @Last Modified time: 2019-03-20 10:49:32
...
export default class TargetSetting extends Component {
  ...
  //表格数据请求
  requestTab(pages) {
    Request('api/web/listenJob/listen_mission_setting', {
      'pageSize': 20,
      'curPage': pages
    }, function (ret) {
      if (ret.result) {
    ...
    const columns = [
      {
        ...
        render: (text, record) => (
          <span>
            <SVG type='icon-chakanxq' onClick={() => this.handleShowDetail(record)} />
            <Link to={{ pathname: '/admin/tpk/sttsz/TpkManaNewTask', state: record }}>
              <SVG type='bianji' />
            </Link>
            <SVG type='shanchu1' onClick={() => this.delcon(record)} />
          </span>
        ),
      }
    ];
    ...
    return (
      <div className='mj-lot-tabCon'>
        ...
        <div className='mj-lot-operate mj-tmlts-operate'>
          <Link to={'/admin/tpk/sttsz/TpkManaNewTask'}>
